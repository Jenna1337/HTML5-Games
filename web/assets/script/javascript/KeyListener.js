...
"NumLock", // The NumLock or Number Lock key, to toggle numer-pad mode function for interpreting subsequent keyboard input.  
...
// Whitespace Keys§
"Enter", // The Enter or ↵ key, to activate current selection or accept current input.
...
"Home", // The Home key, used with keyboard entry, to go to start of content (KEYCODE_MOVE_HOME).
...
"Escape", // The Escape (Esc) key. This key was originally used to initiate an escape sequence, but is now more generally used to exit or "escape" the current context, such as closing a dialog or exiting full screen mode.  
...
"Power", // Toggle power state. (KEYCODE_POWER) 
...
"Dead", // A dead key combining key. It may be any combining key from any keyboard layout. For example, on a PC/AT French keyboard, using a French mapping and without any modifier activiated, this is the key value \u0302 COMBINING CIRCUMFLEX ACCENT. In another layout this might be a different unicode combining key.
...
"Close", // Close the current document or message (// Note: This doesn’t close the application). (APPCOMMAND_CLOSE, KEYCODE_MEDIA_CLOSE)  
...
"MediaPause", // Pause the currently playing media. (APPCOMMAND_MEDIA_PAUSE, KEYCODE_MEDIA_PAUSE) 
...
// The normal 0 ... 9 numpad keys are encoded as "0" ... "9", but some multimedia keypads have buttons numbered from 1 ... 12. In these instances, the 10 key is often labeled 10 /0.
...
"LaunchCalculator", // The "Calculator" key. This is often the generic "LaunchApplication" key, as index 2. (APPCOMMAND_LAUNCH_APP2, KEYCODE_CALCULATOR)  
...
"LaunchMyComputer", // The "My Computer" key. This is often the generic "LaunchApplication" key, as index 1. (APPCOMMAND_LAUNCH_APP1)  
...
"MediaApps", // Show a list of media applications: audio/video players and image viewers. (VK_APPS) 
...
"AudioVolumeDown", // Decrease audio volume. (VK_VOLUME_DOWN)  
"AudioVolumeUp", // Increase audio volume. (VK_VOLUME_UP)  
"AudioVolumeMute", // Toggle between muted state and prior volume level. (VK_VOLUME_MUTE)  
